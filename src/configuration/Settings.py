...
class ClientSettings(BaseSettings):
    url: str = Field(
        ..., 
        description="Backend client URL",
        env="CLIENT__URL"
    )
...
class Settings(BaseSettings):
    ...
    def __init__(self, **kwargs):
        ...
        if env == 'integration':
            ...
            mail_postgres_password = os.environ.get('INTEGRATION_MAIL_POSTGRES_PASSWORD', 'testpass123')
            kwargs.setdefault('database', {
                'url': f"postgresql://lleidahack_mail_user:{mail_postgres_password}@db-mail-integration:5432/lleidahack_mail_integration"
            })
            kwargs.setdefault('client', {
                'url': 'http://backend-integration:8000/',
                'service_token': os.environ.get('CLIENT__SERVICE_TOKEN', 'HOLA')
            })
        else:
            ...
            kwargs.setdefault('client', {
                'url': os.environ.get('CLIENT__URL', 'http://localhost:8000/'),
                'service_token': os.environ.get('CLIENT__SERVICE_TOKEN', 'HOLA')
            })
        ...
        if not kwargs.get('mail'):
            ...
            if env != 'integration' and not all([mail_username, mail_password, mail_from]):
                raise ValueError("MAIL__USERNAME, MAIL__PASSWORD, and MAIL__FROM_MAIL environment variables are required")
            
            kwargs.setdefault('mail', {
                'username': mail_username or 'info@lleidahack.dev',
                'password': mail_password or 'changeme',
                'from_mail': mail_from or 'info@lleidahack.dev',
                'port': int(os.environ.get('MAIL__PORT', 465)),
                'server': os.environ.get('MAIL__SERVER', 'smtp.gmail.com'),
                'from_name': os.environ.get('MAIL__FROM_NAME', 'InfoLleidaHack'),
                'send_mails': os.environ.get('MAIL__SEND_MAILS', 'false').lower() == 'true'
            })
