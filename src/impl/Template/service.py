...
class TemplateService(BaseService):
    ...
    def get_by_id(self, id):
        template = self.db.query(MailTemplate).filter(MailTemplate.id == id).first()
        if not template:
    ...
    def get_template(self, template_id: int, data: TokenData):
        template = self.db.query(MailTemplate).filter(MailTemplate.id == template_id).first()
        if not template:
            raise HTTPException(status_code=404, detail="Template not found")
        return template
    ...
    def update_template(self, template_id: int, template: MailTemplateUpdate, data: TokenData):
    ...
    def update_template_status(self, template_id: int, active_template: MailTemplateActive, data: TokenData): 
        db_template = self.get_by_id(template_id)
